...
EXPERIMENT_NAME = "pretrain_smolvla_bundle_all_k"
TRACKIO_PROJECT = "pretrain-smolvla-bundle-all-k"
CREATED_AT = "2026-08-23T18:20:07+03:00"
...
TRAIN_STEPS_BY_BUDGET = {1: 1_000, 2: 1_500, 3: 2_000, 5: 2_000, 10: 2_000, 25: 2_000}
TRAIN_BATCH_SIZE = 32
...
TRAIN_WORKERS = 16  # 6 concurrent trainings x 16 workers fit the 256-core box
TRAIN_LOG_FREQ = 25
...
OUTPUT_ROOT = Path(
    "/var/tmp/vla_outputs/bundle_all_k_20260823_182007"
)
...
# Bundle ingredient: training-time Gaussian noise on the NORMALIZED
# proprioceptive state inside policy.forward (STATE is MEAN_STD-normalized,
# so alpha equals sigma_i = alpha * Std(s_i) in raw units).  alpha=0.10 was
# the best k=1 setting in the state-noise sweeps; the bundle applies it
# uniformly at every budget.  Dedicated RNG stream, main training RNG
# untouched.  Evaluation applies no noise.
STATE_NOISE_ALPHA = 0.10
NOISE_STREAM_SEED = 91_000

# Concurrency (overridable via VLA_TRAIN_SLOTS / VLA_EVAL_SLOTS env vars):
# per GPU, this many concurrent training and evaluation processes.  A
# full-FT training holds ~30 GB and an evaluation ~12 GB on 144 GB cards.
TRAIN_SLOTS_PER_GPU = 2
EVAL_SLOTS_PER_GPU = 2
