...
def write_outputs(summary: dict[str, Any], output_dir: Path) -> None:
    ...
    line_styles = ["-", "--", ":"]
    styles = {
        str(action_steps): line_styles[index % len(line_styles)]
        for index, action_steps in enumerate(EVAL_ACTION_STEPS)
    }
    ...
    ax_tasks.set(
        ...
        title="Per-task cost curves (n=50/35/25 solid/dashed/dotted)",
    )
