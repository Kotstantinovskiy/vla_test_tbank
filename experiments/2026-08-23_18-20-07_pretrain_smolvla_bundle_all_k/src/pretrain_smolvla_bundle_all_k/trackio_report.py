    ...
    try:
        ...
        payload: dict[str, Any] = {
            ...
            "reports/summary": trackio.Markdown(
                ...
                "Bundle adaptations (full FT + image transforms + state noise "
                "alpha=0.10 + budget-dependent steps) for k=1..25. Each checkpoint "
                "is evaluated cleanly at inference n_action_steps=50/35/25. Full "
                "per-episode videos are stored on disk; this run logs the first "
                "success and first failure per budget and variant."
            ),
        }
        for item in gifs:
