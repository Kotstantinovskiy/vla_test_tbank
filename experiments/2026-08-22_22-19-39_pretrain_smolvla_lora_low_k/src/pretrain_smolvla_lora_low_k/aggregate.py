...
def write_report(summary: dict[str, Any], path: Path) -> None:
    lines = [
        "# LoRA-on-VLM few-shot cost curve on the official-data pretrain",
        "",
        "Every task/budget adaptation starts from the frozen pretrain checkpoint",
        "independently: LoRA adapters (r=16, alpha=32) on every used VLM linear",
        "(text layers 0-14, vision encoder, connector) plus full training of the",
        "action expert and projections (the expert-only baseline's trainable",
        "set). Adapters are merged into a plain checkpoint before evaluation.",
        "Demos are the official demo_0..demo_{k-1} of each task from the in-repo",
        "libero_goal conversion.",
