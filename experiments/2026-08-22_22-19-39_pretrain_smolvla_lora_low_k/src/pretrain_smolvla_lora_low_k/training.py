...
def adapter_model(task_id: int, budget: int) -> Path:
    """The lerobot-train PEFT checkpoint (adapter weights + configs)."""
...
def final_model(task_id: int, budget: int) -> Path:
    """The merged plain-SmolVLA checkpoint consumed by evaluation."""

    return (
        job_output(task_id, budget)
        / f"checkpoints/{TRAIN_STEPS:06d}/pretrained_model_merged"
    )


def training_complete(task_id: int, budget: int) -> bool:
    """Training counts as complete only once the merged checkpoint exists."""

    state = (
        job_output(task_id, budget)
        / f"checkpoints/{TRAIN_STEPS:06d}/training_state/training_step.json"
    )
    adapter = adapter_model(task_id, budget) / "adapter_model.safetensors"
    merged = final_model(task_id, budget) / "model.safetensors"
    if not (state.is_file() and adapter.is_file() and merged.is_file()):
        return False
    return json.loads(state.read_text()).get("step") == TRAIN_STEPS


def adapter_complete(task_id: int, budget: int) -> bool:
    state = (
        job_output(task_id, budget)
        / f"checkpoints/{TRAIN_STEPS:06d}/training_state/training_step.json"
    )
    adapter = adapter_model(task_id, budget) / "adapter_model.safetensors"
    if not (state.is_file() and adapter.is_file()):
        return False
    return json.loads(state.read_text()).get("step") == TRAIN_STEPS
...
def ensure_adapted_manifest(root: Path, task_id: int, budget: int) -> Path:
    ...
    if config.get("use_peft"):
        raise ValueError(f"Merged checkpoint still claims use_peft: {model}")
    ...
    payload = {
        ...
        "adapter_model_safetensors_sha256": _sha256(adapter),
    }
...
def build_command(root: Path, task_id: int, budget: int) -> list[str]:
    ...
    full_training = json.dumps(list(FULL_TRAINING_MODULES), separators=(",", ":"))
    return [
        str(trainer),
        ...
        f"--peft.target_modules={LORA_TARGET_REGEX}",
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Train one LoRA-on-VLM task/budget adaptation and merge it"
    )
    ...
    if training_complete(args.task_id, args.budget):
        ...
        return
    if not adapter_complete(args.task_id, args.budget):
        ...
        if not adapter_complete(args.task_id, args.budget):
            raise RuntimeError(
                "Trainer exited successfully but the adapter checkpoint is incomplete"
            )
    from .merge import merge_adapter

    merge_adapter(args.task_id, args.budget)
    if not training_complete(args.task_id, args.budget):
        raise RuntimeError("Merge finished but the merged checkpoint is incomplete")
    manifest = ensure_adapted_manifest(root, args.task_id, args.budget)
    print(
        json.dumps(
            {
