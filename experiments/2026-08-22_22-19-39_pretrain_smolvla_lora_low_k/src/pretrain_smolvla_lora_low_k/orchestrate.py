...
def require_preflight(root: Path) -> None:
    ...
    peft = audit.get("peft", {})
    if peft.get("method") != "LORA" or not peft.get("target_modules"):
        raise RuntimeError(f"Audit does not describe the LoRA protocol: {peft}")
    if not audit.get("lora_parameter_count") or not audit.get("full_training_parameter_count"):
        raise RuntimeError("LoRA audit is missing adapter or full-training parameters")
