...
from pretrain_smolvla_low_k_deterministic_repro.aggregate import wilson_interval
from pretrain_smolvla_low_k_deterministic_repro.constants import (
    DEMO_BUDGETS,
    EVAL_BATCH_SIZE,
    EVAL_EPISODES,
    MASTER_SEED,
...
from pretrain_smolvla_low_k_deterministic_repro.dataset_smoke import (
    verify_loaded_episode_indices,
)
from pretrain_smolvla_low_k_deterministic_repro.determinism import compare
from pretrain_smolvla_low_k_deterministic_repro.selection import build_manifest
from pretrain_smolvla_low_k_deterministic_repro.training import build_command
from pretrain_smolvla_low_k_deterministic_repro.trackio_report import first_outcome_gifs
...
def test_first_outcome_gifs_picks_first_success_and_failure(tmp_path: Path):
    ...
    assert by_key[(1, "success")]["task_id"] == 2
    assert len(items) == 6
