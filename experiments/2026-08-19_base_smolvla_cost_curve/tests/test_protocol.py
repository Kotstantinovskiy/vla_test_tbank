...
from base_smolvla_cost_curve.aggregate import wilson_interval
from base_smolvla_cost_curve.constants import (
    DEMO_BUDGETS,
    TARGET_ENV_TASK_IDS,
    TARGET_INSTRUCTIONS,
    TRAIN_STEPS,
)
from base_smolvla_cost_curve.selection import build_manifest
from base_smolvla_cost_curve.training import build_command
from base_smolvla_cost_curve.trackio_report import first_outcome_gifs
...
def test_training_command_freezes_vlm_and_uses_official_demos(tmp_path: Path):
    ...
    assert any(
        item == "--policy.path=/var/tmp/vla_outputs/smolvla_base_libero_official_schema_c83c3163"
        for item in command
    )
...
def test_env_task_ids_are_identity():
    ...
    assert sorted(DEMO_BUDGETS) == [1, 2, 3, 5, 10, 25]
...
def test_first_outcome_gifs_picks_first_success_and_failure(tmp_path: Path):
    ...
    assert len(items) == 2 * len(DEMO_BUDGETS)
