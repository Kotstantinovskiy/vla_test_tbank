...
def run(
    ...
    if action_steps not in EVAL_ACTION_STEPS:
    ...
    if cfg.n_action_steps != TRAINED_ACTION_STEPS:
        raise ValueError(
            "Adapted checkpoint no longer carries the trained default "
            f"n_action_steps={TRAINED_ACTION_STEPS}: got {cfg.n_action_steps}"
        )
    # Inference-time override only; the checkpoint config keeps the trained 50.
    ...
    output = result_path(results_root, task_id, alpha, action_steps)
    videos_dir = (
        results_root
        / "videos"
        / f"task_{task_id}"
    ...
    manifest_file = adapted_manifest_path(experiment_root(), task_id, alpha)
    ...
    result = {
        ...
        "eval_noise": "none (augmentation is training-only)",
...
def result_complete(path: Path, action_steps: int) -> bool:
    if not path.is_file():
        return False
    ...
    return (
        ...
        and result.get("n_action_steps") == action_steps
        and result.get("trained_n_action_steps") == TRAINED_ACTION_STEPS
...
def main() -> None:
    ...
    parser.add_argument("action_steps", type=int)
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--results-root", type=Path, default=root / "results/raw")
    parser.add_argument("--force", action="store_true")
    args = parser.parse_args()
    model = final_model(args.task_id, args.alpha)
    output = result_path(args.results_root, args.task_id, args.alpha, args.action_steps)
    if not args.force and result_complete(output, args.action_steps):
        print(json.dumps({"state": "already_complete", "output": str(output)}))
        return
    result = run(
        args.task_id, args.alpha, args.action_steps, model, args.results_root, args.device
    )
