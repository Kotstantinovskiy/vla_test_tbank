...
# External reference: the 2000-step sweep's alpha=0.00 control
# (2026-08-22_22-43-55_pretrain_smolvla_state_noise_k1, same seeds/episodes).
# Context only: this experiment trains for 1000 steps with a proportionally
# compressed LR schedule, so its own alpha=0.00 arm is the valid baseline.
FULL_FT_K1_REFERENCE = {
    "experiment": "2026-08-22_22-43-55_pretrain_smolvla_state_noise_k1 (2000 steps, alpha=0.00)",
    "per_task_successes": {"0": 3, "1": 19, "2": 15},
    "mean_success": 0.6166666666666666,
}
...
def write_outputs(summary: dict[str, Any], output_dir: Path) -> None:
    ...
    ax_tasks.set(
        ...
        title="Per-task success vs alpha (k=1, 1000 steps, n=50; x = 2k-step α=0 ref)",
    )
    ...
    ax_mean.axhline(
        ...
        label="2000-step alpha=0 reference",
    )
    ax_mean.set(
        xlabel="state-noise alpha", ylabel="success rate", ylim=(-0.03, 1.03),
...
def write_report(summary: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            + " |",
            "",
            f"2000-step alpha=0 reference mean (external): {FULL_FT_K1_REFERENCE['mean_success']:.3f}.",
            "",
        ]
    )
