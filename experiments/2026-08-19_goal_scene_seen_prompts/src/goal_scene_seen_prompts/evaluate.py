...
def make_predicate_vec_env(env_cfg: LiberoConfig, point: dict) -> _LazyAsyncVectorEnv:
    ...
    fns = [
        partial(
            ...
            behavior_target=point["behavior_target"],
            **gym_kwargs,
        )
...
def run_point(point: dict, policy_bundle, root: Path, checkpoint_sha: str) -> dict:
    ...
    for episode in per_episode:
        ...
        episode["min_eef_target_dist"] = record["min_eef_target_dist"]
        episode["final_target_displacement"] = record["final_target_displacement"]
        episode["max_target_displacement"] = record["max_target_displacement"]
