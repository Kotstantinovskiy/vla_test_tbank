...
from .constants import DEMO_BUDGETS, GPU_IDS, TARGET_INSTRUCTIONS, experiment_root
...
def main() -> None:
    ...
    def worker(gpu: int) -> None:
        while True:
            ...
            train_command = [
                sys.executable,
                "-m",
                "pretrain_few_shot_low_k.training",
                str(task_id),
                str(budget),
            ]
            ...
            eval_command = [
                sys.executable,
                "-m",
                "pretrain_few_shot_low_k.evaluate",
                str(task_id),
                str(budget),
            ]
    ...
    if run_stage([sys.executable, "-m", "pretrain_few_shot_low_k.aggregate"], aggregate_log, os.environ.copy()):
        raise SystemExit("Aggregation failed")
    trackio_log = root / "results/logs/trackio.log"
    if run_stage([sys.executable, "-m", "pretrain_few_shot_low_k.trackio_report"], trackio_log, os.environ.copy()):
