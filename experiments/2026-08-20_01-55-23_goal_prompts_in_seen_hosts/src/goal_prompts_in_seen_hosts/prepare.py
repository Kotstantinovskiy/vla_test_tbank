...
def benchmark_listing(
    suite: str = SUITE, expected_tasks: int = EXPECTED_SUITE_TASKS
) -> list[dict]:
    ...
    if len(listing) != expected_tasks:
        raise ValueError(
            f"Expected {expected_tasks} {suite} tasks, got {len(listing)}"
        )
    return listing
...
def main() -> None:
    ...
    plan = build_plan(
        benchmark_listing(), benchmark_listing("libero_goal", expected_tasks=10)
    )
    plan.update(
        {
