...
def build_manifest(conversion_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if conversion_manifest["source_revision"] != OFFICIAL_SOURCE_REVISION:
        raise ValueError(
            f"Unexpected source revision {conversion_manifest['source_revision']!r}; "
            f"expected {OFFICIAL_SOURCE_REVISION!r}"
        )
...
def main() -> None:
    ...
    print(
        json.dumps(
            {
                task_id: task["official_demos"][str(max(DEMO_BUDGETS))]
