...
def run(
    ...
    if cfg.chunk_size != TRAINED_CHUNK_SIZE:
        raise ValueError(
            f"Expected trained chunk_size={TRAINED_CHUNK_SIZE}, got {cfg.chunk_size}"
        )
    ...
    try:
        ...
        if descriptions != (expected,):
            raise RuntimeError(
                f"Environment task mismatch: expected={expected!r}, "
                f"actual={descriptions!r}"
            )
        context = (
        ...
        with torch.no_grad(), context:
            for episode_index in range(EVAL_EPISODES):
                ...
                if len(episodes) != 1 or len(raw_videos) != 1:
                    raise RuntimeError(
                        f"Expected one episode/video, got {len(episodes)}/"
                        f"{len(raw_videos)}"
                    )
    ...
    if not manifest_file.is_file():
        raise FileNotFoundError(
            f"Adapted checkpoint manifest missing; run training first: {manifest_file}"
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Evaluate one low-k action-step point")
