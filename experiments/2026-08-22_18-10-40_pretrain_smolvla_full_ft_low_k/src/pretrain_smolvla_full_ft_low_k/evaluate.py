...
def run(
    ...
    output = result_path(results_root, task_id, budget, action_steps)
    videos_dir = (
        results_root
        / "videos"
        / f"task_{task_id}"
...
def main() -> None:
    ...
    output = result_path(args.results_root, args.task_id, args.budget, args.action_steps)
    if not args.force and result_complete(output, args.action_steps):
        print(json.dumps({"state": "already_complete", "output": str(output)}))
        return
    result = run(
        args.task_id, args.budget, args.action_steps, model, args.results_root, args.device
    )
