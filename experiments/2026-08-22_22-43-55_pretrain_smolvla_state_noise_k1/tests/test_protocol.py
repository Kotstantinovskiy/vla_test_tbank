...
from pretrain_smolvla_state_noise_k1.aggregate import wilson_interval
from pretrain_smolvla_state_noise_k1.constants import (
    ALPHAS,
    DEMO_BUDGET,
    DEMO_BUDGETS,
    EVAL_ACTION_STEPS,
...
from pretrain_smolvla_state_noise_k1.dataset_smoke import (
    verify_loaded_episode_indices,
)
from pretrain_smolvla_state_noise_k1.determinism import compare
from pretrain_smolvla_state_noise_k1.selection import build_manifest
from pretrain_smolvla_state_noise_k1.training import build_cli_args, install_state_noise
...
def test_axis_is_alpha_at_k1():
    ...
    assert ALPHAS == (0.0, 0.01, 0.03, 0.05)
    assert EVAL_ACTION_STEPS == (50,)
    assert EVAL_ACTION_STEPS[0] == TRAINED_ACTION_STEPS
    assert alpha_tag(0.0) == "0.00" and alpha_tag(0.05) == "0.05"
    with pytest.raises(ValueError):
        alpha_tag(0.02)
    assert result_path(Path("/r"), 2, 0.03, 50) == Path("/r/task_2/alpha_0.03/n_50.json")
...
def test_cli_args_full_ft_and_alpha_axis(tmp_path: Path):
    ...
    args = build_cli_args(root, 1, 0.03)
    ...
    assert any("alpha_0.03" in item for item in args)
    assert not any(item.startswith("--peft") for item in args)
    with pytest.raises(ValueError, match="alpha"):
        build_cli_args(root, 0, 0.2)
