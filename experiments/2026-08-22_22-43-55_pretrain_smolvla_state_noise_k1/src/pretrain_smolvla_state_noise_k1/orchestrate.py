...
def require_preflight(root: Path) -> None:
    ...
    expected_jobs = len(TARGET_INSTRUCTIONS) * len(ALPHAS)
    # All alpha arms of one task share the single k=1 dataset, so the dataset
    # smoke validates one selection per task.
    if dataset_smoke.get("passed") is not True or len(dataset_smoke["checks"]) != len(
        TARGET_INSTRUCTIONS
    ):
        raise RuntimeError("Dataset selection smoke did not validate every task's k=1 set")
...
def main() -> None:
    ...
    state: dict[str, Any] = {
        ...
        "failed_jobs": 0,
        "jobs": {
            f"task_{task_id}_alpha_{alpha_tag(alpha)}": {
                "task_id": task_id,
                "alpha": alpha,
                "state": "pending",
    ...
    def worker(gpu: int) -> None:
        while True:
            try:
                task_id, alpha = work.get_nowait()
            except queue.Empty:
                return
            key = f"task_{task_id}_alpha_{alpha_tag(alpha)}"
            environment = os.environ.copy()
            environment["CUDA_VISIBLE_DEVICES"] = str(gpu)
            update(key, state="training", gpu=gpu, started_at=now_iso())
            train_log = train_logs / f"{key}.log"
            code = run_stage(
                [sys.executable, "-m", f"{package}.training", str(task_id), str(alpha)],
                train_log,
                environment,
            )
            if code:
            ...
            for action_steps in EVAL_ACTION_STEPS:
                output = result_path(
                    root / "results/raw", task_id, alpha, action_steps
                )
                ...
                update_variant(key, action_steps, state="evaluating")
                code = run_stage(
                    [
                        sys.executable,
                        "-m",
