...
def main() -> None:
    ...
    def worker(gpu: int) -> None:
        while True:
            ...
            train_command = [
                sys.executable,
                "-m",
                "pretrain_few_shot_tune.training",
                str(task_id),
                str(budget),
            ]
            ...
            eval_command = [
                sys.executable,
                "-m",
                "pretrain_few_shot_tune.evaluate",
                str(task_id),
                str(budget),
            ]
    ...
    if run_stage([sys.executable, "-m", "pretrain_few_shot_tune.aggregate"], aggregate_log, os.environ.copy()):
        raise SystemExit("Aggregation failed")
    trackio_log = root / "results/logs/trackio.log"
    if run_stage([sys.executable, "-m", "pretrain_few_shot_tune.trackio_report"], trackio_log, os.environ.copy()):
