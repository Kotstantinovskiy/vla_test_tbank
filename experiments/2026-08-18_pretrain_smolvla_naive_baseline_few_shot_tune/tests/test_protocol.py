...
from pretrain_few_shot_tune.aggregate import wilson_interval
from pretrain_few_shot_tune.constants import (
    DEMO_BUDGETS,
    TARGET_ENV_TASK_IDS,
    TARGET_INSTRUCTIONS,
    TRAIN_STEPS,
)
from pretrain_few_shot_tune.selection import build_manifest
from pretrain_few_shot_tune.training import build_command
from pretrain_few_shot_tune.trackio_report import first_outcome_gifs
...
def test_first_outcome_gifs_picks_first_success_and_failure(tmp_path: Path):
    ...
    assert by_key[(5, "success")]["task_id"] == 3
    assert len(items) == 6
