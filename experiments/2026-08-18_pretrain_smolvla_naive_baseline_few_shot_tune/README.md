# Few-shot cost curve на official-претрене (k=5/10/25)

Дата: 2026-08-18. Кост-кривая нового претрена
(`2026-08-17_smolvla_pretrain_libero`): 10 задач `libero_goal` × бюджеты
k∈{5,10,25} = 30 независимых адаптаций, каждая стартует с базового
чекпойнта.
...
```bash
uv sync --frozen
cd experiments/2026-08-18_pretrain_smolvla_naive_baseline_few_shot_tune
...
- Trackio-проект `pretrain-few-shot-tune`.
