...
# External paired reference: the no-augmentation full-FT experiment
# (2026-08-22_18-10-40_pretrain_smolvla_full_ft_low_k) -- identical recipe,
# seeds, demos, and evaluation; the only difference is
# dataset.image_transforms.enable.  Successes out of 20 per (n, task, k).
FULL_FT_REFERENCE = {
    ...
    "mean_tasks_0_2": {
        "50": {"1": 0.5833333333333334, "2": 0.9500000000000001, "3": 0.7666666666666666},
        "25": {"1": 0.5333333333333333, "2": 0.75, "3": 0.7999999999999999},
    },
}
...
def aggregate(results_root: Path) -> dict[str, Any]:
    ...
    summary = {
        ...
        "full_ft_reference": FULL_FT_REFERENCE,
        "tasks": tasks,
        "cost_curve": {
            "mean_tasks_0_2": {
                str(action_steps): {
...
def write_report(summary: dict[str, Any], path: Path) -> None:
    ...
    for action_steps in EVAL_ACTION_STEPS:
        reference = FULL_FT_REFERENCE["mean_tasks_0_2"][str(action_steps)]
        lines.append(
            f"| n={action_steps} (no aug) | "
            + " | ".join(f"{reference[str(k)]:.3f}" for k in DEMO_BUDGETS)
            + " |"
        )
    lines.append("")
