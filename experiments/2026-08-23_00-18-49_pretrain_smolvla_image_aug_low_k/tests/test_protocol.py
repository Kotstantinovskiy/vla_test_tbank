...
from pretrain_smolvla_image_aug_low_k.aggregate import wilson_interval
from pretrain_smolvla_image_aug_low_k.audit import (
    INTENDED_FROZEN_PATTERNS,
    classify_trainable,
    intended_frozen,
)
from pretrain_smolvla_image_aug_low_k.constants import (
    DEMO_BUDGETS,
    EVAL_ACTION_STEPS,
    EVAL_BATCH_SIZE,
    EVAL_EPISODES,
...
from pretrain_smolvla_image_aug_low_k.dataset_smoke import (
    verify_loaded_episode_indices,
)
from pretrain_smolvla_image_aug_low_k.determinism import compare
from pretrain_smolvla_image_aug_low_k.selection import build_manifest
from pretrain_smolvla_image_aug_low_k.training import build_command
from pretrain_smolvla_image_aug_low_k.trackio_report import first_outcome_gifs
...
def test_training_command_is_full_fine_tune(tmp_path: Path):
    ...
    assert "--steps=1500" in command  # k=2 budget-dependent steps
    command_k1 = build_command(root, 1, 1)
    assert "--steps=1000" in command_k1
    command_k3 = build_command(root, 1, 3)
    assert "--steps=2000" in command_k3
    ...
    assert "--dataset.image_transforms.enable=true" in command
...
def test_seed_protocol_matches_deterministic_repro():
    ...
    assert TRAIN_STEPS_BY_BUDGET == {1: 1_000, 2: 1_500, 3: 2_000}
...
def test_installed_lerobot_image_transform_defaults_match_protocol():
    """The experiment enables lerobot's default transforms verbatim; fail if
    the installed library's defaults drift from the documented protocol."""
    ...
    assert cfg.enable is False  # enabled only via the training CLI flag
    assert cfg.max_num_transforms == IMAGE_TRANSFORMS_MAX_NUM
    assert cfg.random_order is False
    ...
    for name, (expected_type, expected_kwargs) in IMAGE_TRANSFORMS_EXPECTED.items():
        ...
        assert transform.weight == 1.0, name
