...
`artifacts/trackio/`. Корневой launcher автоматически индексирует эти каталоги
через симлинки и открывает один dashboard со всеми проектами:
...
генерируется при запуске и не копирует результаты. Добавленный новый эксперимент
появится после того, как он создаст свою `artifacts/trackio/*.db`. Посмотреть
найденные проекты без запуска UI можно командой `scripts/index_trackio.sh`.
